...
IF "%WIN%"=="32" (
make -w -d -p 32-bit DEBUG=T V=1 SHELL="$SHELL -x" > %BUILDDIR%/32bit.log 2>&1
if %errorlevel% neq 0 (
...
make -w -d -p distribution DEBUG=T V=1 SHELL="$SHELL -x" > %BUILDDIR%/distribution.log 2>&1
if %errorlevel% neq 0 (
...
make -w -d -p check-all V=1 SHELL="$SHELL -x" > %BUILDDIR%/check.log 2>&1
if %errorlevel% neq 0 (
